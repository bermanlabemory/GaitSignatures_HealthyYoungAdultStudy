function [] = Plot3DConditions_PC456(GaitSigs,condname,gaitgroup,spec,conditions,pallette,plotting)
...
for  r = 1:length(U);
    ...
    for k = 1:length(indices)
        plot3(GaitSigs(indices(k),301:400),GaitSigs(indices(k),401:500),GaitSigs(indices(k),501:600),'Color',pointColors(indices(k),:),'LineStyle','-','LineWidth',2,'DisplayName',string(U(r))) % for Python
        hold on
    end  
end
% xlim([-15 20])
