...
%% Figure 1: Representative Plots of Raw Data 
...
%% Figure 1: Plot phase averaged kinematics 
% Plot according to speed using gradient of subject color
...
for feat = 1:60;
    ...
    for i = 1:3
        ...
        for j = 1:length(indices)
            plot(squeeze(PhaseAvgPCs_shift_LHS(indices(j),feat,:)),'Color',ColorCell{1,i}(j,:),'LineWidth',2); %gradient of color across subject speed trials
            %plot(squeeze(PhaseAvgPCs_shift_LHS(indices(j),feat,:)),'Color',colors(i,:),
            %'LineWidth',2) % single color per all subject trials
...
xlabel('Distance between SS and fixed slow signatures'); % change here depending on what is being correlated
...
%% Correlation between SS and extreme slow signature distance change vs. change speed
...
y = FullTable_WithDist.Slow_Fast(f); % change here depending on what is being correlated
[r, p_value] = corrcoef(x, y);
coefficients = polyfit(x, y, 1);
x_values = linspace(min(x), max(x), 100);
y_values = polyval(coefficients, x_values);
...
ylabel('Distance between SS and fixed slow signatures'); % change here depending on what is being correlated
hold on;
...
%% Correlation between Balance vs. speed change extreme fast and slow

f = find(trialtype == 3); %get one instance per subject
colors = [FullTable_WithDist.SubColors(f,1) FullTable_WithDist.SubColors(f,2) FullTable_WithDist.SubColors(f,3)];


x = FullTable_WithDist.SpeedChange_SSSlow(f);
y = FullTable_WithDist.BeamBalance(f);
[r, p_value] = corrcoef(x, y);
coefficients = polyfit(x, y, 1);
x_values = linspace(min(x), max(x), 100);
...
figure();
scatter(x, y, 100, colors, 'filled');
xlabel('Speed change SS to Slow (m/s)');
